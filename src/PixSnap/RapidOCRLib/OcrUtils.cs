...
namespace RapidOCRLib
{
    class OcrUtils
    {
        public static Mat MatFromBgra32(byte[] bgra, int width, int height)
        {
            int expected = width * height * 4;
            if (bgra == null || bgra.Length < expected)
                throw new ArgumentException("Invalid BGRA pixel buffer.");

            using Mat bgraMat = new Mat(height, width, DepthType.Cv8U, 4);
            Marshal.Copy(bgra, 0, bgraMat.DataPointer, expected);
            Mat bgr = new Mat();
            CvInvoke.CvtColor(bgraMat, bgr, ColorConversion.Bgra2Bgr);
            return bgr;
        }
        ...
        public static Mat GetRotateCropImage(Mat src, List<Point> box)
        {
            ...
            Rectangle rect = new Rectangle(left, top, cropW, cropH);
            ...
            imgCropWidth = Math.Max(1, imgCropWidth);
            imgCropHeight = Math.Max(1, imgCropHeight);
