...
namespace RapidOCRLib
{
    public partial class OcrLite
    {
        ...
        /// <summary>
        /// OCR Detect — BGRA32 像素缓冲（无 PNG 编解码，适合 WPF 截图）。
        /// </summary>
        public async Task<OcrResult> DetectAsyncFromBgra32(
            byte[] bgraPixels,
            int width,
            int height,
            int padding = 50,
        ...
        {
            Mat originSrc = OcrUtils.MatFromBgra32(bgraPixels, width, height);
            return await DetectCoreAsync(padding, maxSideLen, boxScoreThresh, boxThresh, unClipRatio, doAngle, mostAngle, originSrc);
        }
        ...
        /// <summary>
        /// 仅运行识别模型（跳过检测），用于裁切后的单行文字。
        /// </summary>
        public async Task<TextLine> RecognizeLineFromBgra32Async(byte[] bgraPixels, int width, int height)
        {
            return await Task.Run(() =>
            {
                using Mat originSrc = OcrUtils.MatFromBgra32(bgraPixels, width, height);
                var lines = crnnNet.GetTextLines(new List<Mat> { originSrc });
                return lines.Count > 0 ? lines[0] : new TextLine();
            });
        }
    }
}
